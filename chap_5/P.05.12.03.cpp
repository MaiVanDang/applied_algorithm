...
//Cập nhật các khả năng đi từ i đến các đỉnh khác thỏa mãn điều kiện giới hạn d
//Và cập nhật giá khi đi từ i đến các khả năng thỏa mãn
void BFS(int i){
    queue<int> Q;
...
//Xây dựng đồ thị và cập nhật các giá thành thông qua BFS
void buildGraph(){
    for(int i = 1; i <= n; i++)
        for(int j = 1; j <= n; j++)
            w[i][j] = INF;
